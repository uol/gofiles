...
func TestZip(t *testing.T) {
	...
	compressed, err := gofiles.ZipBytes(largeString)
...
func TestUnzip(t *testing.T) {
	...
	compressedBytes, err := gofiles.ZipBytes([]byte(decompressedString))
	if !assert.NoError(t, err, "error compressing bytes") {
		return
	}

	decompressedBytes, err := gofiles.UnzipBytes(compressedBytes)
